...
enyo.kind({
	...
	getAppPath: new enyo.webOS.ServiceRequest({
		service: "palm://com.palm.applicationManager/", method: "getAppBasePath"
	}),
	...
	create: function() {
		...
		this.appPath = "";
		this.checkLoadHitched = enyo.bind(this, "checkLoad");
	...
	appChanged: function() {
		this.appPath = "";
		if (this.app) {
			this.$.getAppPath.go({appId:this.app});
			this.getAppPath.response(this.gotAppInfo);
		} else {
	...
	gotAppInfo: function(inSender, response) {
		if(!response || !response.returnValue) {
			this.error("Could not get app path: "+(response && response.errorText));
			return;
		}
	...
	pathChanged: function() {this.log("crossappui path changed...")
		var targetPath = "";
		...
		if (this.path) {
			...
			if (targetPath) {
				this.log("CrossAppUI: Loading cross-app UI at "+targetPath);
	...
	checkLoad: function() {
		...
		if(doc && doc.readyState === "complete" && doc.location.href === "about:blank" && this.path) {
			...
			this.log("CrossAppUI: checkLoad: things look okay.");
		}
	},
